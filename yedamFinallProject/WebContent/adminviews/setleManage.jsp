...
<body>
	<form id="frm" name="frm" method="post" action="AdminSetleManageOk.ad" onsubmit="return fromCheck()">
					...
					<div class="card-body"> <!-- 첫번째 카드 바디 시작 -->
						<div class="row ">
							...
							<div class="col-md-2 pt-2 row-st">
								<select class="custom-select row-st" id="ctg1" name="ctg1">
									<option value="o_group" selected>주문그룹번호</option>
									<option value="setleId">주문번호</option>
						...
						<div class="row ">
							...
							<div class="col-md-2 pt-2 row-st">
								<select class="custom-select row-st" id="ctg2" name="ctg2">
						...
						<div class="row ">
							...
							<div class="col-md-2 row-st">
								<select class="custom-select" id="orderTime" name="orderTime">
						...
						<div class="row ">
							<div class="col-md-1 pt-3 row-st">주문상태</div>
							<div class="col-md-1 row-st">
							...
							<div class="col-md-1 row-st">
								<div class="custom-control custom-checkbox">
								  <input type="checkbox" class="custom-control-input" id="cancel" name="chk" value="complete">
								  <label class="custom-control-label" for="cancel">결제완료</label>
								</div>
							</div>
							<div class="col-md-1 row-st">
								<div class="custom-control custom-checkbox">
								  <input type="checkbox" class="custom-control-input" id="refund" name="chk" value="refund">
								  <label class="custom-control-label" for="refund">환불</label>
								</div>
							</div>
		                  ...
		                  <table class="table table-hover table-condensed">
		                    <thead class="text-primary">
		                    	<tr align="center">
								  ...
								  <th>결제금액</th>
								  <th>결제상태</th>								  
								  <c:if test="${stlDto.stat.equals('refund') }"><th>결제상태</th></c:if>
							  	</tr>
		                    </thead>
		                    <tbody align="center">
		                    <c:forEach var="map" items="${list }">
								<tr class="text-center">
									...
									<td>${map.get("order_group_no") }</td>
									<td>${map.get("order_id") }</td>
									<td>${map.get("user_id") }</td>
									<td>${map.get("order_setlede") }</td>
									<td>${map.get("order_setlede_time") }</td>
									<td>${map.get("order_qy") }</td>
									<td>${map.get("goods_name") }</td>
									<td>${map.get("setle_amount") }</td>
									<c:if test="${map.get('setle_code').equals('SC') }"><td>결제완료</td></c:if>
									<c:if test="${map.get('setle_code').equals('CC') }"><td>환불</td></c:if>
	...
	<script>	
	 $( "#startDate" ).datepicker({
	  ...
	  //현재 페이지 수 submit
		...
		//검색 조건 고정
	    if(${stlDto != null}){		
	    	$("select[name='ctg1']").val(['${stlDto.sCtg}']);
	    	$("[name='s_word1']").val(['${stlDto.name1}']);
	    	$("[name='ctg2']").val(['${stlDto.sCtg2}']);
	    	$("[name='s_word2']").val(['${stlDto.name2}']);
	    	$("[name='startDate']").val(['${stlDto.sDate}']);
	    	$("[name='endDate']").val(['${stlDto.eDate}']);
	    	$("[name='orderTime']").val(['${stlDto.time}']);				    		 	 	 
	    	$("[name='chk']").val(['${stlDto.stat}']); 	
	    	$("[name='sort']").val(['${stlDto.sort}']);
